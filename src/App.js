...
function ResultItems({ raw }) {
  ...
  return (
    <div style={{ background:"#fff", border:"1px solid #e8d5c0", borderRadius:"4px", padding:"6px 9px" }}>
      {items.map((r,i)=>(
        <div key={i} style={{ display:"flex", gap:"6px", alignItems:"flex-start", marginBottom:i<items.length-1?"4px":"0" }}>
          <span style={{ color:"#8b5e3c", fontWeight:"700", fontSize:"10px", flexShrink:0, marginTop:"2px" }}>{i+1}.</span>
          <span style={{ fontSize:"11px", color:"#5c3317", lineHeight:"1.6", fontWeight:"500" }}>{r.trim()}</span>
        </div>
      ))}
    </div>
  );
...
function ResultEditor({ gi, 결과, update }) {
  ...
  const removeItem = ii => { const arr=[...items]; arr.splice(ii,1); commit(arr); };
      ...
      {items.map((item,ii)=>(
        <div key={ii} style={{ display:"flex", gap:"4px", marginBottom:"4px", alignItems:"center" }}>
          <input value={item} onChange={e=>updateItem(ii,e.target.value)}
            style={{ flex:1, background:"#fff8f0", border:"1px solid #c4a882", borderLeft:"3px solid #5c3317",
              borderRadius:"3px", color:"#5c3317", fontWeight:"500", padding:"4px 6px", fontSize:"10px",
              fontFamily:"inherit", outline:"none" }} />
          <button type="button" onPointerDown={e=>{ e.stopPropagation(); e.preventDefault(); removeItem(ii); }}
            style={{ background:"none", border:"none", color:"#c0703a", cursor:"pointer", fontSize:"14px", padding:"0 4px" }}>✕</button>
        </div>
      ))}
      <button type="button" onPointerDown={e=>{ e.stopPropagation(); e.preventDefault(); addItem(); }}
        style={{ fontSize:"9px", padding:"4px 10px", background:"#f5ede4", border:"1px dashed #c4a882",
          borderRadius:"4px", color:"#8b5e3c", cursor:"pointer", marginTop:"2px", display:"block" }}>+ 결과물 추가</button>
    </div>
  );
}
...
function WorkPanel({ name, partColor, workData, setWorkData, reqData, setReqData }) {
  ...
  const saveTask = () => {
    ...
    setWorkData(prev=>{
      ...
      if(editIdx!==null){
        const pt=list[editIdx];
        list[editIdx]={...form,이력:[...(pt.이력||[]),{시각:now,변경전상태:pt.상태,변경후상태:form.상태,메모:form.리뷰}]};
      } else {
  ...
  const addLog=(idx,memo)=>{
    setWorkData(prev=>{
      const list=[...(prev[name]||[])]; const t=list[idx];
      const now=new Date().toISOString().slice(0,16).replace("T"," ");
  ...
  const changeStatus=(idx,newState)=>{
    setWorkData(prev=>{
      const list=[...(prev[name]||[])]; const t=list[idx];
      const now=new Date().toISOString().slice(0,16).replace("T"," ");
      list[idx]={...t,상태:newState,완료일:newState==="완료"?today:t.완료일,이력:[...(t.이력||[]),{시각:now,변경전상태:t.상태,변경후상태:newState,메모:`상태 변경: ${t.상태} → ${newState}`}]};
      const next={...prev,[name]:list}; dbSet('work',name,{tasks:list}); return next;
    });
  };
  const deleteTask=i=>{
    setWorkData(prev=>{
      const list=[...(prev[name]||[])]; list.splice(i,1);
      const next={...prev,[name]:list}; dbSet('work',name,{tasks:list}); return next;
    });
  };
  const startEdit=i=>{ setForm({...tasks[i]}); setEditIdx(i); setShowForm(true); };
  const effState=t=>{
    if(!t.상태||t.상태==="진행중") return "진행중";
    if(t.상태==="완료") return "완료";
    if(t.상태==="보류") return "보류";
    if(t.상태==="취소") return "취소";
    if(t.상태==="검토중") return "검토중";
    return "진행중";
  };
  const filtered=(filterState==="전체"?tasks:tasks.filter(t=>effState(t)===filterState))
    ...
    <div style={{ width:"50%", display:"flex", flexDirection:"column", background:"#fdf8f4", borderLeft:"1px solid #d4b896" }}>
      <div style={{ padding:"12px 20px", borderBottom:"2px solid #d4b896", background:"#f0e6d8", flexShrink:0 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"8px" }}>
          ...
          <button onClick={()=>{ setForm({...EMPTY_FORM,접수일:today}); setEditIdx(null); setShowForm(v=>!v); }}
            style={{ padding:"4px 12px", background:showForm?"#c4a882":partColor, color:"#fff", border:"none", borderRadius:"12px", fontSize:"11px", fontWeight:"700", cursor:"pointer" }}>
            {showForm?"✕ 닫기":"+ 신규 업무"}</button>
        </div>
        <ReqSection name={name} reqData={reqData} setReqData={setReqData} partColor={partColor} />
        <div style={{ position:"relative", marginBottom:"6px" }}>
          <input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="업무명, 요청부서, 리뷰 검색..."
            style={{ width:"100%", padding:"5px 28px 5px 8px", border:"1px solid #d4b896", borderRadius:"6px", fontSize:"11px", color:"#3b1f0a", background:"#faf6f1", boxSizing:"border-box", outline:"none" }} />
        ...
        <div style={{ display:"flex", gap:"4px", flexWrap:"wrap" }}>
          {["전체",...상태목록].map(s=>{
            const cnt=s==="전체"?tasks.length:(counts[s]||0); const isA=filterState===s;
            return <button key={s} onClick={()=>setFilterState(s)}
              style={{ padding:"2px 8px", border:"none", borderRadius:"10px", cursor:"pointer", fontSize:"9px", fontWeight:isA?"700":"400",
                ...
                outline:isA?`1px solid ${s==="전체"?"#8b5e3c":(상태색[s]||"#c4a882")}`:"none" }}>{s} ({cnt})</button>;
          })}
        </div>
      </div>
      {showForm&&(
        <div style={{ padding:"14px 18px", borderBottom:"2px solid #c4a882", background:"#fff8f2", flexShrink:0 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"6px", marginBottom:"6px" }}>
            <div style={{ gridColumn:"1/-1" }}>
              ...
              <input value={form.업무} onChange={e=>setForm(p=>({...p,업무:e.target.value}))} placeholder="업무 내용을 입력하세요"
                style={{ width:"100%", padding:"5px 8px", border:"1px solid #c4a882", borderRadius:"4px", fontSize:"12px", color:"#3b1f0a", background:"#faf6f1", boxSizing:"border-box", outline:"none" }} />
            </div>
            <div>
              <div style={{ fontSize:"9px", color:"#a08060", marginBottom:"2px" }}>요청부서</div>
              <input value={form.요청부서} onChange={e=>setForm(p=>({...p,요청부서:e.target.value}))} placeholder="예) 품질팀"
                style={{ width:"100%", padding:"5px 8px", border:"1px solid #d4b896", borderRadius:"4px", fontSize:"11px", color:"#3b1f0a", background:"#faf6f1", boxSizing:"border-box", outline:"none" }} />
            </div>
            <div>
              ...
              <select value={form.상태} onChange={e=>setForm(p=>({...p,상태:e.target.value}))}
                style={{ width:"100%", padding:"5px 8px", border:"1px solid #d4b896", borderRadius:"4px", fontSize:"11px", color:"#3b1f0a", background:"#faf6f1", outline:"none" }}>
                {상태목록.map(s=><option key={s}>{s}</option>)}
              </select>
            </div>
            {["접수일","목표일","완료일"].map(k=>(
              <div key={k}>
                <div style={{ fontSize:"9px", color:"#a08060", marginBottom:"2px" }}>{k}</div>
            ...
            <div style={{ gridColumn:"1/-1" }}>
              ...
              <textarea value={form.리뷰} onChange={e=>setForm(p=>({...p,리뷰:e.target.value}))} placeholder="진행 특이사항, 결과 등" rows={2}
                style={{ width:"100%", padding:"5px 8px", border:"1px solid #d4b896", borderRadius:"4px", fontSize:"11px", color:"#3b1f0a", background:"#faf6f1", boxSizing:"border-box", resize:"vertical", fontFamily:"inherit", outline:"none" }} />
            </div>
          </div>
          <div style={{ display:"flex", gap:"6px", justifyContent:"flex-end" }}>
            <button onClick={()=>{ setShowForm(false); setForm(EMPTY_FORM); setEditIdx(null); }}
              style={{ padding:"5px 14px", background:"#e8d5c0", border:"none", borderRadius:"10px", fontSize:"11px", color:"#6b4226", cursor:"pointer", fontWeight:"600" }}>취소</button>
            <button onClick={saveTask}
              style={{ padding:"5px 16px", background:partColor, border:"none", borderRadius:"10px", fontSize:"11px", color:"#fff", cursor:"pointer", fontWeight:"700" }}>
              {editIdx!==null?"수정 저장":"등록"}</button>
          </div>
        </div>
      )}
      <div style={{ flex:1, overflowY:"auto", padding:"14px 18px" }}>
        ...
        {[...filtered].reverse().map((t,fi)=>{
          const realIdx=tasks.indexOf(t); const es=effState(t);
          const bc=상태색[es]||"#8b6a4a"; const bg=상태배경[es]||"#f0e6d8";
          return (
            <div key={fi} onClick={()=>setDetailIdx(realIdx)}
              style={{ background:"#ffffff", border:"1px solid #d4b896", borderLeft:`3px solid ${bc}`, borderRadius:"5px", padding:"9px 11px", marginBottom:"7px", cursor:"pointer" }}
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"4px" }}>
                <div style={{ fontSize:"12px", fontWeight:"600", color:"#3b1f0a", flex:1, lineHeight:"1.4", marginRight:"6px" }}>
                  <span style={{ color:"#c4a882", fontWeight:"700", marginRight:"5px", fontSize:"11px" }}>#{realIdx+1}</span>{t.업무}
                </div>
      ...
      {detailIdx!==null&&tasks[detailIdx]&&(
        <DetailModal task={tasks[detailIdx]} taskIdx={detailIdx} partColor={partColor}
  ...
  if(!text.trim()&&!editing) return (
    <div style={{ marginBottom:"8px" }}>
      <button onClick={()=>{ setDraft(text); setEditing(true); }}
        style={{ fontSize:"9px", color:"#d4842a", background:"none", border:"1px dashed #e8a44a", borderRadius:"4px", padding:"3px 10px", cursor:"pointer" }}>+ 팀장 요청사항 추가</button>
    </div>
  );
  return (
    <div style={{ background:"#fff3e0", border:"1px solid #e8a44a", borderLeft:"3px solid #d4842a", borderRadius:"5px", padding:"8px 12px", marginBottom:"8px" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"5px" }}>
        ...
        {!editing?<button onClick={()=>{ setDraft(text); setEditing(true); }}
          style={{ fontSize:"9px", color:"#d4842a", background:"#ffe0b2", border:"none", borderRadius:"6px", padding:"2px 8px", cursor:"pointer", fontWeight:"600" }}>수정</button>
        :<div style={{ display:"flex", gap:"4px" }}>
          ...
          <button onClick={()=>{ setReqData(p=>({...p,[name]:draft})); setEditing(false); }}
      ...
      {!editing?<div style={{ fontSize:"10px", color:"#5a3a0a", lineHeight:"1.8" }}>
...
function ResultModal({ cat, allData, onClose }) {
  ...
  useEffect(()=>{ dbGet('merged',cat).then(d=>{ if(d?.mergedMap) setMergedMap(d.mergedMap); if(d?.editMerge) setEditMerge(d.editMerge); }); },[cat]);
  useEffect(()=>{ if(Object.keys(mergedMap).length>0||Object.keys(editMerge).length>0) dbSet('merged',cat,{mergedMap,editMerge}); },[mergedMap,editMerge,cat]);
  ...
  const getMergedRows = gi => {
    ...
    merged.forEach((idxArr,mi)=>{
      ...
      result.push({ type:"merged", mi, gi, members, text:editMerge[`${gi}_${mi}`]!==undefined?editMerge[`${gi}_${mi}`]:members.map(m=>m.결과).join("\n") });
    });
    rows.forEach((r,i)=>{ if(!usedIdx.has(i)) result.push({ type:"single", idx:i, gi, row:r }); });
    return result;
  };
  const getDoneCount=(g,gi)=>{ if(!getRows(g).length) return 0; return getMergedRows(gi).length; };
  const handleMerge=gi=>{ if(selected.filter(s=>s.gi===gi).length<2) return; const idxArr=selected.filter(s=>s.gi===gi).map(s=>s.idx); setMergedMap(p=>({...p,[gi]:[...(p[gi]||[]),idxArr]})); setSelected(p=>p.filter(s=>s.gi!==gi)); };
  const handleUnmerge=(gi,mi)=>{ setMergedMap(p=>{ const a=[...(p[gi]||[])]; a.splice(mi,1); return {...p,[gi]:a}; }); setEditMerge(p=>{ const n={...p}; delete n[`${gi}_${mi}`]; return n; }); };
      ...
      <div style={{ background:"#faf6f1", borderRadius:"12px", width:"640px", maxWidth:"94vw", maxHeight:"88vh", display:"flex", flexDirection:"column", boxShadow:"0 10px 40px rgba(91,51,23,0.3)" }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ padding:"12px 20px 10px", borderBottom:"1px solid #d4b896", display:"flex", gap:"8px", flexWrap:"wrap" }}>
          {groups.map((g,gi)=>{
            ...
            return <button key={gi} onClick={()=>setActiveGroup(isActive?"all":gi)}
            ...
            return (
              <div key={gi} style={{ marginBottom:"16px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"7px" }}>
                  ...
                  {selForGroup.length>=2&&<button onClick={()=>handleMerge(gi)}
                    style={{ padding:"2px 10px", background:color, border:"none", borderRadius:"10px", fontSize:"10px", color:"#fff", cursor:"pointer", fontWeight:"700" }}>✂ {selForGroup.length}개 묶기</button>}
                ...
                {mergedRows.length===0?<div style={{ fontSize:"11px", color:"#c4a882", padding:"8px 0" }}>결과물 없음</div>:(
                  <div style={{ display:"flex", flexDirection:"column", gap:"6px" }}>
                    {mergedRows.map((item,ii)=>{
                        ...
                        <div key={ii} style={{ background:"#fff8f0", border:`1.5px solid ${color}`, borderLeft:`4px solid ${color}`, borderRadius:"6px", padding:"10px 12px" }}>
                          <div style={{ display:"flex", alignItems:"center", gap:"5px", marginBottom:"6px", flexWrap:"wrap" }}>
                            ...
                            <button onClick={()=>handleUnmerge(gi,item.mi)}
                              style={{ marginLeft:"auto", padding:"1px 8px", background:"#fdf0eb", border:"none", borderRadius:"8px", fontSize:"9px", color:"#c0703a", cursor:"pointer", fontWeight:"600" }}>묶기 해제</button>
                          </div>
                          <div style={{ background:"#fff", border:"1px solid #e8d5c0", borderRadius:"4px", padding:"6px 9px" }}>
                      ...
                      return (
                        <div key={ii} onClick={()=>toggleSelect(gi,item.idx)}
                          style={{ background:isSelected?"#f5ede4":"#faf6f1", border:`1.5px solid ${isSelected?color:"#e8d5c0"}`, borderLeft:`3px solid ${isSelected?color:"#c4a882"}`, borderRadius:"6px", padding:"9px 12px", cursor:"pointer" }}>
                          <div style={{ display:"flex", alignItems:"center", gap:"5px", marginBottom:"5px" }}>
                            <input type="checkbox" checked={isSelected} readOnly style={{ cursor:"pointer", accentColor:color }} />
...
function Dashboard({ allData, allWork, onNavigate }) {
  ...
  useEffect(()=>{
    const calc = () => {
      ...
      const leftH = leftRef.current.getBoundingClientRect().height;
      const qcMembers = allData["QC"].length;
      const rowH = Math.max(24, Math.floor((leftH - 28 - 24) / qcMembers));
      setQcRowH(rowH);
    };
...
export default function App() {
  ...
  const [allData, setAllData] = useState(()=>{
    const d={};
    Object.entries(MEMBERS).forEach(([p,pd])=>{ d[p]=pd.members.map(m=>({...m,goals:m.goals.map(g=>({...g,실적:0,비고:"",결과:""}))})); });
    return d;
  });
    ...
    const load=async()=>{
      try {
        ...
        for(const [pk,pd] of Object.entries(MEMBERS)){
          newData[pk]=await Promise.all(pd.members.map(async m=>{ const saved=await dbGet('goals',m.name); return {...m,goals:m.goals.map((g,gi)=>({...g,...(saved?.[gi]||{})}))}; }));
        }
        setAllData(newData);
        const newWork={};
  ...
  const update=(gi,field,val)=>{
    setAllData(prev=>{
      const next={...prev};
      next[activePart]=next[activePart].map((m,mi)=>mi!==activeMember?m:{...m,goals:m.goals.map((g,idx)=>idx!==gi?g:{...g,[field]:field==="실적"?Math.min(Number(val),g.배점):val})});
      const mem=next[activePart]?.[activeMember];
      if(mem){ const gd={}; mem.goals.forEach((g,gi)=>{ gd[gi]={실적:g.실적||0,비고:g.비고||'',결과:g.결과||''}; }); dbSet('goals',mem.name,gd); }
      return next;
    });
  };
    ...
    <div style={{ fontFamily:"'Noto Sans KR','Malgun Gothic',sans-serif", background:"#faf6f1", color:"#3b2a1a", display:"flex", flexDirection:"column", position:"fixed", top:0, left:0, right:0, bottom:0, overflow:"hidden", boxSizing:"border-box" }}>
      <div style={{ background:"linear-gradient(90deg,#5c3317,#3b1f0a)", borderBottom:"1px solid #8b5e3c", padding:"12px 28px", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
        ...
        <div style={{ display:"flex", gap:"6px", alignItems:"center" }}>
          {["전략","업무","개인"].map(cat=>{
            const cnt=Object.values(allData).flat().reduce((s,m)=>s+m.goals.filter(g=>g.cat===cat&&g.배점>0&&(g.결과||"").trim()).length,0);
            return <button key={cat} onClick={()=>setResultModal(cat)}
          ...
          <button onClick={()=>{ const bk={allData,allWork,allReq,savedAt:new Date().toISOString()}; navigator.clipboard.writeText(JSON.stringify(bk)).then(()=>alert("✅ 백업 완료!\n클립보드에 복사됐습니다.")).catch(()=>alert("클립보드 복사 실패")); }}
            style={{ padding:"4px 8px", background:"rgba(255,255,255,0.08)", border:"1px solid rgba(255,255,255,0.15)", borderRadius:"10px", color:"#c9a880", fontSize:"13px", cursor:"pointer" }}>BK</button>
          <button onClick={()=>{ setRestoreText(""); setRestoreModal(true); }}
            style={{ padding:"4px 8px", background:"rgba(255,255,255,0.08)", border:"1px solid rgba(255,255,255,0.15)", borderRadius:"10px", color:"#c9a880", fontSize:"13px", cursor:"pointer" }}>RS</button>
        </div>
      </div>
      <div style={{ display:"flex", background:"#f0e6d8", borderBottom:"1px solid #c4a882", flexShrink:0 }}>
        <button onClick={()=>setActiveTab("dashboard")}
          style={{ padding:"10px 20px", border:"none", cursor:"pointer", background:activeTab==="dashboard"?"#ffffff":"transparent", color:activeTab==="dashboard"?"#5c3317":"#8b6a4a", fontWeight:activeTab==="dashboard"?"700":"400", fontSize:"13px", borderBottom:`2px solid ${activeTab==="dashboard"?"#5c3317":"transparent"}` }}>
          📊 대시보드
        </button>
        <div style={{ width:"1px", background:"#c4a882", margin:"8px 0" }} />
        {Object.entries(MEMBERS).map(([key,pd])=>{
          const pm=allData[key]; const avg=Math.round(pm.reduce((s,m)=>s+calcRate(m.goals),0)/pm.length); const isA=activeTab==="detail"&&activePart===key;
          return <button key={key} onClick={()=>{ setActiveTab("detail"); setActivePart(key); setActiveMember(0); }}
            style={{ flex:1, padding:"10px 12px", border:"none", cursor:"pointer", background:isA?"#ffffff":"transparent", color:isA?pd.color:"#5c3317", fontWeight:isA?"700":"400", fontSize:"13px", borderBottom:`2px solid ${isA?pd.color:"transparent"}` }}>
          ...
          <>
            <div style={{ width:"160px", minWidth:"160px", background:"#f0e6d8", borderRight:"1px solid #c4a882", overflowY:"auto" }}>
              {members.map((m,i)=>{ const r=calcRate(m.goals); const isA=activeMember===i;
                return <button key={m.name} onClick={()=>setActiveMember(i)}
                  style={{ display:"block", width:"100%", padding:"12px 14px", textAlign:"left", border:"none", cursor:"pointer", background:isA?"#ffffff":"transparent", borderLeft:`3px solid ${isA?part.color:"transparent"}`, borderBottom:"1px solid #d4b896" }}>
                      ...
                      <div key={gi} style={{ background:"#ffffff", border:"1px solid #d4b896", borderLeft:`3px solid ${CAT_COLOR[g.cat]}`, borderRadius:"5px", padding:"12px 14px", marginBottom:"8px" }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:"6px", marginBottom:"5px" }}>
                          ...
                          <input type="number" min="0" max={g.배점} value={g.실적||0}
                            onChange={e=>update(gi,"실적",Math.min(Math.max(0,Number(e.target.value)),g.배점))}
