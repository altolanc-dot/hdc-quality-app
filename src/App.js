  ...
  const doSDU = async () => {
    ...
    for(const item of ITEMS){
      ...
      try {
        ...
        // 자료(서로 다른 팀원)가 2건 이상인데 done이 1개뿐이면 강하게 한 번 더 요청
        if(normalized.done.length<2 && raw.length>=2){
          const retryPrompt = `${prompt}\n\n[재요청] 방금 응답이 done 1개였는데, 위 자료에는 서로 다른 팀원의 내용이 ${raw.length}건 있습니다. 그중 내용이 겹치지 않는 2개를 반드시 찾아서 done을 2개 이상으로 다시 응답하세요. 정말 불가능할 때만(자료가 사실상 전부 동일 내용의 반복일 때만) 1개를 유지하세요.`;
          try {
