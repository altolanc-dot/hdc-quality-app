...
const MEMBERS = {
  QC:{ label:"QC 파트", color:"#8b5e3c", members:[
    ...
    { name:"김효진", 전략:50, 업무:50, 개인:0, goals:[
      { cat:"전략", 과제:"예측기반의 타겟점검 운영", 배점:25, 내용:"파트 내 타겟점검 데이터 모니터링 및 운영 지원" },
      { cat:"전략", 과제:"건설 DX 운영 및 전파", 배점:25, 내용:"건설 DX 프로세스 실무 적용 및 피드백 취합" },
      { cat:"업무", 과제:"골조/타일 하자비용 저감 지원", 배점:50, 내용:"품질점검 데이터 정리 및 부적합사항 조치율 추적 관리" },
    ]},
...
const AI_WORK_LINKS = [
  ...
  {id:"qcRestricted", icon:"📊", title:"품질점검 종합 현황판", desc:"품질점검 종합 현황판", href:"https://qc-dashboard-gold.vercel.app/"},
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(45,20,60,0.6)",zIndex:4000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:"#faf6f1",borderRadius:"12px",width:"560px",maxWidth:"94vw",boxShadow:"0 10px 40px rgba(88,28,135,0.35)",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
        <div style={{padding:"14px 20px",background:"linear-gradient(90deg,#581c87,#a855f7)",borderRadius:"12px 12px 0 0",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
...
function SummaryModal({ allData, onClose }) {
  ...
  const ITEMS = [
    {num:1,cat:"전략",label:"예측기반 타겟점검",score:20,desc:"예측기반 타겟점검 운영안 수립·시행",match:t=>t.includes("예측기반")||t.includes("타겟점검")||t.includes("전기공종 타겟")||t.includes("레미콘"),
      focus:"QPI 개정 관련 내용을 서술할 때 \"전기\" 공종은 비중이 크지 않으니 절대 언급하지 말 것. 건축·설비 등 다른 공종 중심으로 QPI 개정 내용을 종합적으로 서술할 것. 미진행 이슈는 실행률·달성률처럼 시간이 지나면 채워지는 지표가 아니라, 현장에서 COP(자율점검)매니저를 활용한 점검 체계가 아직 정착되지 못하고 있다는 점을 있으면 우선 반영할 것."},
  ...
  const doSDU = async () => {
    ...
    for(const item of ITEMS){
      ...
      const sourceCount = raw.reduce((s,r)=>s + r.결과.length + ((r.비고||"").trim()?1:0), 0);
      const focusLine = item.focus ? `\n\n[우선순위 지침] ${item.focus}\n위 지침에 해당하는 내용이 있으면 done/pending 선별 시 최우선으로 반영할 것.` : "";
      const prompt = `품질팀 "${item.label}" 목표(${item.score}점) 팀원 실적:\n\n${dataText}${focusLine}\n\n두 가지를 각각 추려서 JSON으로 응답. 실적(done)이 핵심이고 미진행(pending)은 보조 정보이니, 분량 비중을 done 위주로 둘 것.\n\n[done] 경영진 보고용 핵심 성과. 이 규칙은 예외를 두지 말고 반드시 지킬 것: 자료(결과물+진행현황)에 서로 다른 내용이 2건 이상 있으면 done은 절대 1개로 응답하면 안 되고 반드시 2개 이상(최대 3개) 반환할 것. "서로 다른 내용"은 꼭 다른 팀원일 필요 없이, 한 사람이 "결과물"을 여러 건 올렸다면 그 여러 건도 서로 다른 성과로 쳐서 각각 별도 done 항목으로 나눠 쓸 것 — 한 문장에 두 가지 성과(예: "A를 완료하여 B를 확립")가 섞여 있다면 이것도 A와 B로 쪼개서 2개로 만들 것. 정말로 자료 전체에 쪼갤 수 있는 내용이 단 1건뿐일 때만 예외적으로 1개 허용. "결과물" 항목은 이미 승인·확정된 산출물이므로 최우선 반영 대상. "결과물"만으로 2~3개를 못 채우면 "진행현황"의 주요 내용을 반드시 끌어와서라도 채울 것(부족하다고 1개로 줄이지 말 것). 각 항목은 단순히 무엇을 했는지 나열하지 말고, 그 결과물이 "${item.label}"이라는 목표 달성에 어떻게 기여했는지 연계해서 서술할 것.\n[pending] 배열 원소는 반드시 0개 또는 정확히 1개여야 함(2개 이상 절대 금지). "현재 실제로 진행 중이면서 막혀있거나 협의·조치가 필요한 사안" 중 가장 시급하거나 대표적인 것 딱 하나만 선정. 자격 조건: 구체적인 원인(데이터 오류, 시스템 결함, 타 부서 협의 필요, 일정 조율 필요 등)이 명시되어 있어야 함. 다음은 반드시 제외: (1) 단순히 "~진행 중"이라고만 쓰인 정상 진행 항목, (2) "계획 대비 실행률/달성률 저조", "연간 점검 횟수 미달" 등 시간이 지나 정기 활동을 계속 수행하면 자연히 채워지는 누적형 수치·비율 지표(실제 장애물이 아니라 단순 시점상의 진행률이므로 이슈가 아님). 이 기준에 맞는 항목이 없으면 빈 배열.\n\n공통 규칙: 중복 제거, 제목 15자 이내. 담당자 이름(사람 이름)은 절대 포함하지 말 것 — 업무 내용만 서술. done의 설명(desc)은 65~75자 분량의 한 문장으로(예시 기준: "Dooray 프로젝트방 운영·품질레터 공유 체계 수립, Snowflake 기반 아이클릭 데이터 연동 및 CS 대시보드 개선 진행 중" 정도의 길이) 작성하되, 이 글자 수는 반드시 "2줄에 꽉 차는 분량"이라는 목표이지 그 이상을 억지로 채우라는 뜻이 아님. 화면에 표시될 폰트/너비 기준으로 정확히 2줄에 들어가는 분량으로 스스로 요약을 조절할 것 — 3줄이 되거나 잘리지 않도록 반드시 2줄 이내로 압축. 60자 미만은 지양. pending의 설명(desc)은 30~45자 분량 한 줄로 짧게, 무엇이 왜 막혀있는지 구체적으로. 모든 설명(desc)은 반드시 명사형으로 종결(예: "~완료", "~구축", "~진행 중", "~협의 필요", "~보류" 등)하고 마침표는 붙이지 않음. "~했습니다", "~합니다", "~했다" 같은 서술형 종결어미는 절대 사용하지 않음. 모든 항목이 동일한 종결 스타일과 분량을 갖도록 통일.\nJSON만 응답: {"done":[{"title":"제목","desc":"설명"}],"pending":[{"title":"제목","desc":"설명"}]}`;
      try {
        const data = await callAPI(prompt);
        const text = (data.content?.[0]?.text||"{}").replace(/```json|```/g,"").trim();
        const parsed = JSON.parse(text);
        let normalized = {done: Array.isArray(parsed.done)?parsed.done:[], pending: (Array.isArray(parsed.pending)?parsed.pending:[]).slice(0,1)};

        // 원본 자료 안의 개별 성과 건수(결과물 항목 수 + 진행현황 존재 여부 합)가 2건 이상인데 done이 1개뿐이면 강하게 한 번 더 요청
        if(normalized.done.length<2 && sourceCount>=2){
          const retryPrompt = `${prompt}\n\n[재요청] 방금 응답이 done 1개였는데, 위 자료에는 쪼갤 수 있는 개별 성과가 최소 ${sourceCount}건 있습니다(같은 사람이 올린 여러 결과물도 포함). 한 문장에 두 가지 성과가 섞여 있다면 나눠서라도 done을 2개 이상으로 다시 응답하세요. 정말 불가능할 때만(내용이 사실상 하나의 사실을 반복 서술한 것뿐일 때만) 1개를 유지하세요.`;
          try {
            const data2 = await callAPI(retryPrompt);
            const text2 = (data2.content?.[0]?.text||"{}").replace(/```json|```/g,"").trim();
            const parsed2 = JSON.parse(text2);
            if(Array.isArray(parsed2.done) && parsed2.done.length>=2){
...
export default function App() {
  ...
  return (
    <div style={{fontFamily:"'Noto Sans KR','Malgun Gothic',sans-serif",background:"#faf6f1",color:"#3b2a1a",display:"flex",flexDirection:"column",position:"fixed",top:0,left:0,right:0,bottom:0,overflow:"hidden",boxSizing:"border-box"}}>
      ...
      {showAIWork&&<AIWorkModal onClose={()=>setShowAIWork(false)} />}
