...
function SummaryModal({ allData, onClose }) {
  ...
  const doSDU = async () => {
    ...
    for(const item of ITEMS){
      ...
      const prompt = `품질팀 "${item.label}" 목표(${item.score}점) 팀원 실적:\n\n${dataText}\n\n경영진 보고용 핵심 성과 3개 이내 요약.\n규칙: 완료 결과물 또는 주요 진행사항 중심, 중복 제거, 제목 15자 이내. 설명(desc)은 40~55자 내외의 한 문장으로, 배경/방법과 결과를 함께 담아 카드 안에서 2줄을 고르게 채우도록 작성(너무 짧아 1줄로 끝나거나 3줄로 넘치지 않게). 모든 설명(desc)은 반드시 명사형으로 종결(예: "~완료", "~구축", "~진행 중", "~수립" 등)하고 마침표는 붙이지 않음. "~했습니다", "~합니다", "~했다" 같은 서술형 종결어미는 절대 사용하지 않음. 모든 항목이 동일한 종결 스타일과 분량을 갖도록 통일.\nJSON만 응답: [{"title":"제목","desc":"설명"}]`;
      try {
